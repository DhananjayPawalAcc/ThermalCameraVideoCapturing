...
namespace ThermoCamSDK
{
    public partial class MainForm
    {
        ...
        */
        ...
        #region CameraPreview With Videowriter
        ...
        private void frameCaptureThread()
        {
            ...
            string saveDirectory = @"D://INTERNSHIP-WORKSPACED";
            string videoFilePath = System.IO.Path.Combine(saveDirectory, $"NEWoutput{videoCount}.mp4");

            // Ensure the directory exists
            System.IO.Directory.CreateDirectory(saveDirectory);

            // Initialize the video writer
            VideoWriter videoWriter = new VideoWriter(videoFilePath, FourCC.MP4V, 30, new OpenCvSharp.Size(pictureBox_Preview.Width, pictureBox_Preview.Height));

            // Check if the VideoWriter is opened successfully
            if (!videoWriter.IsOpened())
            {
                MessageBox.Show("Failed to open video writer", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
                        ...
                        if (frame != null)
                        {
                            Invoke(new Action(() =>
                            {
                                ...
                                if (bmp != null)
                                {
                                    ...
                                    try
                                    {
                                        // Convert Bitmap to Mat and write to video
                                        Mat matFrame = BitmapConverter.ToMat(bmp);
                                        ...
                                        {
                                            MessageBox.Show("VideoWriter is not opened or is null.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                        }
                                    }
                                    catch (Exception writeEx)
                                    {
                                        MessageBox.Show($"Error writing frame to video: {writeEx.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    }
                                }
                            }));
                        }
